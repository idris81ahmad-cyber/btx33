...
const signInRes = await fetch(`${base}/api/auth/callback/credentials`, {
  ...
  body: new URLSearchParams({ csrfToken, username, password, json: "true", callbackUrl: `${base}/admin` }),
});
cookies = [cookies, getCookies(signInRes.headers)].filter(Boolean).join("; ");

const statusRes = await fetch(`${base}/api/admin/storage-status`, { headers: { Cookie: cookies } });
console.log("storage-status:", statusRes.status, await statusRes.text());

const payload = {
  name: "Storage Test " + Date.now(),
  category: "Ankara Prints",
  price: 15000,
  shortDescription: "Testing storage",
  description: "Testing storage write",
  inStock: 5,
  colorFamily: "Gold",
